...
pub fn domain_create(bus: u8, dev: u8, func: u8) -> Result<DomainId, DomainError> {
    ...
    serial::write_fmt(format_args!(
        ...
        dev,
        func,
        unsafe { IOVA_ALLOC.base },
        unsafe { IOVA_ALLOC.limit }
    ))
    .ok();
...
pub fn domain_destroy(domain: DomainId) -> Result<(), DomainError> {
    ...
    serial::write_fmt(format_args!("[iommu] domain={} destroyed\n", domain.0)).ok();
...
#[cfg(all(
    feature = "iommu",
    any(selftest_IOMMU_PROBE, selftest_IOMMU_DENY_DEFAULT)
))]
pub mod selftest {
    use super::*;
            ...
            Some(caps) => {
                match caps.vendor {
                    IommuVendor::Intel => {
                        ...
                        if caps.dma_remapping {
                            serial::write_str(
                                "[PASS: IOMMU_PROBE] Intel DMA remapping available\n",
                            );
                            qemu_exit(0x00); // Success
                        } else {
                            serial::write_str(
                                "[FAIL: IOMMU_PROBE] Intel DMAR found but no DMA remapping\n",
                            );
                    ...
                    IommuVendor::Amd => {
                        serial::write_str(
                            "[SKIP: IOMMU_PROBE] AMD IVRS found but not implemented\n",
                        );
