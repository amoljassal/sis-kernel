...
pub fn trigger_divide_by_zero() -> ! {
    ...
    unsafe {
        ...
        asm!("xor rcx, rcx", "mov rax, 1", "div rcx", options(noreturn));
    }
}
...
fn deep_recurse(n: usize) -> ! {
    ...
    unsafe {
        core::ptr::read_volatile(&(n as u64));
    }
    deep_recurse(n + 1)
}
...
pub fn trigger_ring3_syscall() -> ! {
    ...
    unsafe {
        ...
        // mov rax, 0x01
        *code_ptr.offset(offset) = 0x48;
        offset += 1; // REX.W prefix
        *code_ptr.offset(offset) = 0xC7;
        offset += 1; // MOV r/m64, imm32
        *code_ptr.offset(offset) = 0xC0;
        offset += 1; // ModR/M for RAX
        *code_ptr.offset(offset) = 0x01;
        offset += 1; // immediate 0x01
        *code_ptr.offset(offset) = 0x00;
        offset += 1;
        *code_ptr.offset(offset) = 0x00;
        offset += 1;
        *code_ptr.offset(offset) = 0x00;
        ...
        // int 0x80
        *code_ptr.offset(offset) = 0xCD;
        offset += 1; // INT
        *code_ptr.offset(offset) = 0x80;
        offset += 1; // 0x80
        ...
        *code_ptr.offset(offset) = 0xC0;
        offset += 1;
        *code_ptr.offset(offset) = 0x02;
        offset += 1; // immediate 0x02
        *code_ptr.offset(offset) = 0x00;
        offset += 1;
        *code_ptr.offset(offset) = 0x00;
        offset += 1;
        ...
        *code_ptr.offset(offset) = 0xCD;
        offset += 1;
        *code_ptr.offset(offset) = 0x81;
        offset += 1;

        // hlt (should not reach)
        *code_ptr.offset(offset) = 0xF4;
        offset += 1;
    }
...
pub fn run_sequence() -> ! {
    ...
    #[cfg(selftest_SYSCALL)]
    {
        return run_one("SYSCALL");
    }
    #[cfg(selftest_TIMER)]
    {
        return run_one("TIMER");
    }
    #[cfg(selftest_RING3)]
    {
        return run_one("RING3");
    }
    #[cfg(selftest_RING3_RT)]
    {
        return run_one("RING3_RT");
    }
