...
pub fn run_usr_init() -> Result<(), &'static str> {
    ...
    if !vfs::available() {
        ...
        unsafe {
            crate::arch::x86_64::io::qemu_exit(0x7F);
        } // Skip code
    }
...
pub fn run_vfio_msi_smoke() -> Result<(), &'static str> {
    ...
    serial::write_str(
        "[selftest] NOTE: BAR0 nudge would trigger MSI (not implemented in this test)\n",
    );
    serial::write_str(
        "[selftest] In full test, writing to device register would generate interrupt\n",
    );
