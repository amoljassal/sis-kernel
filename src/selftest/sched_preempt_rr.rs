...
pub fn run() {
    ...
    serial::write_str(" quantum_ticks="); serial::write_dec(RR_QUANTUM_TICKS as u64);
    serial::write_str(" initial_ticks="); serial::write_dec(pc.ticks.load(Ordering::Relaxed));
    serial::write_str(" initial_rr_ticks="); serial::write_dec(pc.rr_ticks.load(Ordering::Relaxed) as u64);
    serial::write_str("\n");
    ...
    while pc.ticks.load(Ordering::Relaxed) - start_ticks < 20 {
        ...
        if current_rr < last_rr_ticks {
            quantum_resets += 1;
            serial::write_str("  quantum reset "); serial::write_dec(quantum_resets as u64); serial::write_str("\n");
        }
    ...
    serial::write_str(" final_ticks="); serial::write_dec(final_ticks);
    serial::write_str(" final_counter="); serial::write_dec(final_counter as u64);
    serial::write_str(" quantum_resets="); serial::write_dec(quantum_resets as u64);
    serial::write_str("\n");
