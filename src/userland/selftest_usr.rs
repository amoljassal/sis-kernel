...
pub fn run_usr_spawn_two() {
    ...
    let pid1 = match validate_spawn("/bin/hello") {
        Ok(p) => p,
        Err(_) => {
...
pub fn run_usr_elf_edges() {
    ...
    let good = vfs::open_elf_verified("/bin/hello")
        .ok_or(())
        .and_then(|img| elfsec::validate_elf64(img).map_err(|_| ()).map(|_| ()));
    if good.is_err() {
        serial::write_str("[selftest] USR_ELF_EDGES: good elf rejected\n");
        unsafe {
            qemu_exit(FAIL_ELF_EDGES_GOOD);
        }
    }
