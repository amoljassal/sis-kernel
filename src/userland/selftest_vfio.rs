...
pub fn run_vfio_roundtrip_test() {
    ...
    serial::write_str(
        "[vfio-test] Testing: ARM → TRIGGER → VERIFY IRQ → DISARM → VERIFY SILENCE\n",
    );
    ...
    serial::write_fmt(format_args!(
        "[vfio-test] Using test handle 0x{:04x}\n",
        test_handle.as_u16()
    ))
    .ok();
    ...
    match vfio::syscall_domain_create(test_handle.as_u16()) {
        Ok(domain_id) => {
            serial::write_fmt(format_args!(
                "[vfio-test] Domain {} created successfully\n",
                domain_id
            ))
            .ok();
        }
    ...
    match vfio::syscall_msi_trigger_e1000_new(test_handle) {
        Ok(()) => {
            serial::write_str(
                "[vfio-test] e1000 trigger sent - checking for interrupt delivery...\n",
            );
    ...
    serial::write_str(
        "[vfio-test] WARNING: Reached post-trigger code - interrupt may not have fired\n",
    );
    ...
    match vfio::syscall_msi_trigger_e1000_new(test_handle) {
        ...
        Err(_) => {
            serial::write_str(
                "[vfio-test] Note: Second trigger failed (expected if MSI disabled)\n",
            );
        }
    }
    ...
    serial::write_str(
        "[vfio-test] If you see this, MSI was properly armed, but IRQ may not have fired\n",
    );
