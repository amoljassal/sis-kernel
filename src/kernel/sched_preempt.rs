...
pub fn on_timer_tick() {
    ...
    let need = pc.need_resched.swap(false, Ordering::AcqRel) || t >= RR_QUANTUM_TICKS;
...
pub fn log_tick_marker() {
    ...
    if (tk & 0x3f) == 0 {
        serial::write_str("[tick] cpu=");
        serial::write_dec(pc.cpu_id as u64);
        serial::write_str(" ticks=");
        serial::write_dec(tk as u64);
        serial::write_str("\n");
    }
}
