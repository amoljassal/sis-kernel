...
// Import centralized kernel types with Ord implementations
use crate::kernel::types::{EdgeId, DesignIPBlockVersion, KernelResult, KernelError};
...
// EdgeId is now imported from crate::kernel::types with Ord implementation

/// Design version for snapshot-based versioning
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, PartialOrd, Ord)]
pub struct DesignVersion(pub u64);

/// IP block version identifier for design graph
/// Note: This is different from crate::kernel::types::DesignIPBlockVersion
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub struct DesignDesignIPBlockVersion {
    pub vendor: String,
    pub name: String,
    pub version: String, // Semantic versioning: "2.1.3"
}

impl PartialOrd for DesignDesignIPBlockVersion {
    fn partial_cmp(&self, other: &Self) -> Option<core::cmp::Ordering> {
        Some(self.cmp(other))
    }
}

impl Ord for DesignDesignIPBlockVersion {
...
pub enum HardwareNode {
    ...
    /// IP block reference
    IPBlock {
        ip_version: DesignIPBlockVersion,
...
pub struct IPBlockRegistry {
    ...
    ip_blocks: BTreeMap<DesignIPBlockVersion, IPBlockEntry>,
    /// Usage statistics
    usage_stats: BTreeMap<DesignIPBlockVersion, IPUsageStats>,
}
...
impl IPBlockRegistry {
    ...
    pub fn register_ip_block(&mut self, version: DesignIPBlockVersion, entry: IPBlockEntry) -> Result<(), IPRegistryError> {
    ...
    pub fn lookup_ip_block(&mut self, version: &DesignIPBlockVersion) -> Option<&IPBlockEntry> {
